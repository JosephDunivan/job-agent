...
### Key Discovery: Crawlee Framework (Apify)

[Crawlee for Python](https://crawlee.dev/python/) is an open-source web crawling framework (Apache-2, 20k+ stars) that **already solves most of the infrastructure we were planning to build ourselves** for browser automation. It sits as an orchestration layer above Playwright/Camoufox and provides:

| Capability | What Crawlee Provides | What We Were Going to Build |
|-----------|----------------------|----------------------------|
| **Request queue** | Persistent, deduplicated URL queue with retry logic. Survives restarts. | Custom SQLite-backed job queue |
| **Auto-scaling** | Concurrency automatically adjusts to system resources (CPU/memory). | Manual concurrency tuning |
| **Proxy rotation** | Smart proxy pool with health monitoring. Blocked proxies auto-removed. Tiered proxy support (try free → fall back to premium). | Custom proxy rotation logic |
| **Session management** | Session pool with cookie persistence per session. Automatic rotation on blocks. | Custom cookie save/load to SQLite |
| **Dataset storage** | Structured data storage with push/export to JSON/CSV. Auto-numbered. | Custom data pipeline to SQLite |
| **Key-value store** | Persistent storage for screenshots, configs, state. | Custom file management |
| **Fingerprint generation** | Realistic browser fingerprints generated by default. | Reliance on Camoufox alone |
| **Camoufox integration** | Official `CamoufoxPlugin` for `PlaywrightCrawler`. Drop-in stealth. Apify provides a ready-made template. | Custom `browser_hand.py` wrapper |
| **Error handling** | Automatic retries, configurable max retries, request-level error isolation. | Custom retry/backoff decorators |
| **Link discovery** | `enqueue_links()` with CSS selectors, include/exclude filters, crawl depth control. | Custom link extraction logic |

**This is a major scope reduction.** Crawlee eliminates the need to build our own request queue, proxy manager, session handler, auto-scaler, and data pipeline. We write the extraction logic; Crawlee handles the plumbing.

#### Crawlee + Camoufox Example (from official docs)
...
class CamoufoxPlugin(PlaywrightBrowserPlugin):
    """Stealth browser plugin using Camoufox engine."""
    @override
...
async def main() -> None:
    ...
    async def request_handler(context: PlaywrightCrawlingContext) -> None:
        ...
        await context.enqueue_links(selector='.next-page')

    await crawler.run(['https://example-jobboard.com/jobs'])
    await crawler.export_data('jobs.csv')
```

#### Crawlee vs. Building Our Own

| Approach | Lines of Code | Features We Get | Maintenance Burden |
|----------|--------------|-----------------|-------------------|
| **Raw Camoufox + custom plumbing** | ~500-800 | Basic browser control | High — we maintain queue, proxy rotation, retry logic, session management, data export |
| **Crawlee + Camoufox plugin** | ~100-200 | All of the above + auto-scaling, fingerprints, persistent storage | Low — Apify maintains the framework (active, well-funded company) |

### Recommended Approach: Crawlee as the Crawling Framework + Stealth Engine Selection

Crawlee replaces most of what `browser_hand.py` was going to be. Instead of building a custom abstraction layer, we use Crawlee's `BrowserPool` plugin system to swap stealth engines per task.

**Architecture with Crawlee:**

```
┌─────────────────────────────────────────────┐
│          Crawlee Framework                  │
│  (request queue, auto-scaling, proxy mgmt,  │
│   session pool, dataset storage, retries)   │
├─────────────────────────────────────────────┤
│                                             │
│  ┌─────────────┐  ┌───────────────────────┐ │
│  │ BrowserPool │  │  Storage Layer        │ │
│  │             │  │                       │ │
│  │ Plugins:    │  │ - Dataset (JSON/CSV)  │ │
│  │ • Camoufox  │  │ - RequestQueue        │ │
│  │ • Playwright│  │ - KeyValueStore       │ │
│  │ • (Nodriver │  │                       │ │
│  │   via       │  │ Bridges to our SQLite │ │
│  │   custom)   │  │ for persistent data   │ │
│  └─────────────┘  └───────────────────────┘ │
└─────────────────────────────────────────────┘
```

**Engine selection by use case (updated):**

| Use Case | Stealth Needed | Engine | Crawlee Crawler Type |
|----------|---------------|--------|---------------------|
| Job board scraping (fallback) | High | Camoufox via `CamoufoxPlugin` | `PlaywrightCrawler` |
| Career page crawling | Medium | Default Playwright + fingerprints | `PlaywrightCrawler` |
| Application form filling | High | Camoufox via `CamoufoxPlugin` | `PlaywrightCrawler` (Phase 2) |
| OSINT data collection | Medium | Nodriver (standalone, outside Crawlee) | Direct `nodriver` usage |
| Simple HTTP fetching (APIs, RSS) | None | No browser needed | `HttpCrawler` or `BeautifulSoupCrawler` |
| Dashboard screenshot/export | None | Vanilla Playwright | Standalone Playwright |
| Testing / CI | None | Vanilla Playwright | pytest + Playwright |

**What Crawlee does NOT replace:**
- Nodriver remains useful for quick one-off stealth tasks outside the crawling framework (OSINT lookups, interactive sessions)
- Our `token_tracker.py` pattern for Ollama integration (different domain)
- The orchestration layer (n8n / LangGraph) that triggers crawls
- Our SQLite data model (Crawlee's Dataset is for raw scraped data; our DB is for processed/enriched data)

### Build vs. Integrate (Updated)

| Component | Previous Plan | Updated Plan |
|-----------|--------------|-------------|
| Crawling framework | **Build** — custom `browser_hand.py` | **Integrate** — Crawlee for Python |
| Request queue + dedup | **Build** — custom SQLite queue | **Integrate** — Crawlee `RequestQueue` |
| Auto-scaling | **Build** — manual tuning | **Integrate** — Crawlee auto-scaling |
| Proxy rotation | **Build** — custom rotation logic | **Integrate** — Crawlee `ProxyConfiguration` |
| Session/cookie management | **Build** — custom save/load | **Integrate** — Crawlee session pool |
| Browser fingerprints | **Rely on** Camoufox only | **Integrate** — Crawlee fingerprint generator + Camoufox |
| Stealth engine: heavy | **Integrate** — Camoufox | **Integrate** — Camoufox via Crawlee `CamoufoxPlugin` |
| Stealth engine: quick OSINT | **Integrate** — Nodriver | **Integrate** — Nodriver (standalone) |
| Scraped data export | **Build** — custom pipeline | **Integrate** — Crawlee `Dataset` + `export_data()` |
| Screenshot capture | **Build** — custom | **Integrate** — Crawlee `KeyValueStore` |
| Action logging / audit trail | **Build** — custom | **Build** — thin wrapper over Crawlee events + our SQLite |
| Error handling / retries | **Build** — custom backoff | **Integrate** — Crawlee built-in retries |

### Installation Requirements (Updated)

```bash
# Crawlee framework + Playwright support + Camoufox
pip install "crawlee[playwright,camoufox]" nodriver

# Camoufox custom Firefox binary
python -m camoufox fetch

# Playwright browser binaries (Crawlee uses Playwright under the hood)
playwright install chromium firefox

# For headless servers (virtual display for headed-mode Camoufox)
sudo apt install -y xvfb libgtk-3-0 libx11-xcb1 libasound2
```

### Apify Platform (Optional, Future)

Crawlee can run standalone (our plan for MVP) or deploy to [Apify's cloud platform](https://apify.com). If we ever need managed infrastructure — larger-scale scraping, built-in proxy pools, scheduled runs, cloud storage — we can deploy our Crawlee crawlers as Apify Actors without rewriting code. This is a free escape hatch, not a dependency.

### Phase: 1 (Crawlee + Camoufox for job/career page crawling), 2 (form filling + application workflows), 3 (full autonomous pipeline)
